	...
	public boolean addWord(String word)
	{
	    ...
	    TrieNode currNode = root;
	    	...
	    	char currChar = word.charAt(i);
	    		...
	    		Set<Character> currChars = currNode.getValidNextCharacters();
	    		if(currChars.contains(currChar)) {
	    			currNode = currNode.getChild(currChar);
		    			if(currNode.getText().equals(word) && !currNode.endsWord()) {
	...
	public boolean isWord(String s) 
	{
	    ...
	    s= s.toLowerCase();
	    	for(int i = 0; i < s.length(); i++) {
	    		...
	    		Set<Character> currChilds = currTrie.getValidNextCharacters();
	    			if(currTrie.getText().equals(s) && currTrie.endsWord()) {
	    				return true;
	    			}
	    			if(currChilds.contains(currChar)) {
	    				currTrie = currTrie.getChild(currChar);
	    			}
	    			else {
	    				return false;
	    			}
